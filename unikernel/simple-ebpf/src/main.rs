...
use aya_ebpf::{
    helpers::{bpf_printk, bpf_probe_read_user_str_bytes},
...
fn try_aya_tracepoint_echo_open(ctx: &TracePointContext) -> Result<u32, i64> {
    ...
    if let Ok(filename_addr) = unsafe { ctx.read_at::<u64>(FILENAME_OFFSET) } {
        ...
        if filename.len() < MAX_PATH {
            ...
            // info!(
            //     ctx,
            //     "Kernel tracepoint sys_enter_openat called, filename {}:{}",
            //     filename.len(),
            //     filename
            // );
            unsafe {
                bpf_printk!(
                    b"tp sys_enter_openat called, filename: %s\n",
        ...
        let filename = unsafe {
            ...
            core::str::from_utf8_unchecked(
                ...
                match bpf_probe_read_user_str_bytes(filename_addr as *const u8, &mut buf) {
                    ...
                    Err(e) => {
                        bpf_printk!(
                            b"tracepoint sys_enter_openat called buf_probe failed %ll",
                            e
                        );
                        return Err(e);
                    }
